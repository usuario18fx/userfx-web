...
export function FxAccessModal({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      if (step === "username" || (step === "identity" && identityEntryReady)) {
  ...
  const stepLabel = step === "username" ? "STEP 1 · TELEGRAM USERNAME" : step === "identity" ? "STEP 2 · SPECIAL CODE" : "STEP 3 · PRIVATE ACCESS";
  ...
  return createPortal(
    <div className="smkl-modal" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) handleClose(); }}>
      <style>{`
        ...
        .smkl-form__submit--get-in{border-color:#63ffd0!important;background:linear-gradient(180deg,#79ffd8,#45e5bd)!important;box-shadow:0 12px 28px #45e5bd24,inset 0 1px 0 #ffffff66!important;}
        .smkl-form__submit--get-in span{color:#0d4ea6!important;text-shadow:none!important;font-weight:800!important;}
      ...
      <div ref={stageRef} className="smkl-modal__stage" id={modalId} role="dialog" aria-modal="true" aria-labelledby={`${modalId}-title`} aria-busy={busy} onMouseDown={(event) => event.stopPropagation()}>
        <button type="button" className="smkl-modal__close" onClick={handleClose} aria-label="Close access" disabled={busy}><span/><span/></button>

        <div className="smkl-modal__bubble smkl-modal__bubble--screen">
          <div className="smkl-typing-viewport"><span className="smkl-typing-text">{bubbleText}</span></div>
          <div className="smkl-screen-userfx"><div className="smkl-modal__brand-line"/><strong>USER🜲FX</strong><div className="smkl-modal__brand-line"/></div>
        </div>
        ...
        <div className="smkl-robot" aria-hidden="true">
          ...
          <div className="smkl-robot__head">
            ...
            <div className="smkl-robot__face"><span className="smkl-robot__eye"/><span className="smkl-robot__eye"/><span className="smkl-robot__mouth"/></div>
          </div>
        </div>
        ...
        <section className="smkl-panel">
          ...
          <div className="smkl-modal__brand-line" />
          <div className="smkl-panel__divider" aria-hidden="true"><span/><RoseIcon/><span/></div>

          <form className="smkl-form" onSubmit={handleSubmit} noValidate>
            <p style={{ margin: 0, color: "#ffffff7a", fontSize: ".72rem", letterSpacing: ".16em" }}>{stepLabel}</p>

            {(step === "identity" || step === "access") && <p className="smkl-identity-user">{username}</p>}
            ...
            {step === "identity" && identityEntryReady && (
              <div className="smkl-form__field">
                ...
                <input ref={primaryInputRef} id={`${modalId}-identity-code`} type="text" name="identity-code" placeholder="SPCL-XXXX" value={identityCode} onChange={(event) => { setIdentityCode(normalizeIdentityCode(event.target.value)); setError(null); }} autoComplete="one-time-code" autoCapitalize="characters" autoCorrect="off" spellCheck={false} maxLength={9} disabled={busy} required />
              </div>
            )}
            ...
            {step === "identity" && !identityEntryReady && (
              <button type="button" className="smkl-form__submit" onClick={openTelegramIdentity} disabled={busy}><span>GET SPECIAL CODE</span></button>
            )}

            {(step !== "identity" || identityEntryReady) && (
              <button type="submit" className={`smkl-form__submit${step === "access" ? " smkl-form__submit--get-in" : ""}${busy ? " is-loading" : ""}`} disabled={busy}>
                <span>{busy ? "CHECKING..." : step === "username" ? "KEEP GOING" : step === "identity" ? "VERIFY IDENTITY" : "GET IN"}</span>
              </button>
            )}
          </form>
        </section>
