...
{/* ========   VERIFIED TELEGRAM =========================== */}
          ...
          <input type="text" value={telegramUsername.replace(/^@/, "")} readOnly aria-label="Verified Telegram username"/>
...
{/* ═════════ SECONDARY FIXED STAGE ═════════ */}
        ...
        .map((plan) => (
        <button key={plan} type="button" role="option" aria-selected={prefix === plan} className={prefix === plan ? "is-selected" : ""} onClick={() => {
        setPrefix(plan);
        setPrefixMenuOpen(false);
        setError("");
        }}>
        ...
        {error && (
        <p className="pvr-direct-error" role="alert">{error}</p>
        )}
        ...
        <article key={plan.id} className={`pvr-direct-plan is-${plan.id} ${selectedPlan === plan.id ? "is-selected" : ""}`} role="button" tabIndex={0} onClick={() => setSelectedPlan(plan.id)} onKeyDown={(event) => {
