...
const ICONS = {
  anydevice: "/assets/iconos/anydevice.png",
    ...
    groups: [
      {
        ...
        items: [
          {
            icon: ICONS.fotos,
            text: "Private session access",
            sub: "Unlock one selected session from the vault",
          },
          {
            icon: ICONS.getin,
            text: "Instant entry",
            sub: "Your access code is generated after purchase",
          },
          {
            icon: ICONS.anydevice,
            text: "Any device",
            sub: "Watch smoothly on phone, tablet or desktop",
          },
          {
            icon: ICONS.preview,
            text: "Full session view",
            sub: "Enjoy the complete unlocked session, not just a preview",
          },
        ],
      },
      {
        label: "Best for",
        items: [{ icon: ICONS.candado2, text: "One premium drop", sub: "One-time access only." }],
      },
    ],
  },
  pro: {
        ...
        items: [
          {
            ...
            sub: "Pick from available premium sessions in the vault",
          },
          {
            icon: ICONS.quality,
            text: "Enhanced quality",
            sub: "Higher-quality streaming for a smoother viewing experience",
          },
          {
            icon: ICONS.quickly,
            text: "Instant access",
            sub: "No waiting — unlock and enter when you are ready",
          },
        ],
      },
      {
        ...
        items: [
          {
            icon: ICONS.fuego,
            text: "Regular viewers",
            sub: "Made for people who want more than one session",
          },
          { icon: "💎", text: "More value per entry", sub: "Code saved. Access secured." },
          {
            icon: ICONS.vipcard,
            text: "VIP-ready",
            sub: "A natural step up before going all-access",
          },
        ],
      },
    ],
    ...
    groups: [
      {
        ...
        items: [
          ...
          { icon: ICONS.limit, text: "Unlimited sessions", sub: "Enter as many times as you want" },
          {
            ...
            text: "No day limit",
            sub: "Unlimited entries while your VIP code remains active",
          },
      ...
      {
        label: "",
        items: [{ icon: ICONS.telegram, text: "Chat Experience", sub: "Direct DM for anything" }],
      },
    ],
  },
};
...
export default function VaultDevice() {
  ...
  const [selectedPlan, setSelectedPlan] = useState<keyof PlanKey | null>(null);
  ...
  return (
    <div className="vd-device vd-device-damask">
      ...
      <div className="vd-screen">
          ...
          <div className="vd-menu-options">
            <div
              ...
              <div className="vd-option-row">
                ...
                <div className="vd-option-info">
                  <div className="vd-option-name">Basic</div>
                </div>
            ...
            <div
              ...
              <div className="vd-option-row">
                ...
                <div className="vd-option-info">
                  <div className="vd-option-name">PRO</div>
                </div>
                <div className="vd-option-arrow">›</div>
              </div>
            </div>
            <div
              ...
              <div className="vd-option-row">
                ...
                <div className="vd-option-info">
                  <div className="vd-option-name">VIP</div>
                </div>
                <div className="vd-option-arrow">›</div>
              </div>
            </div>
        ...
        {/* ── Detail Sheet ── */}
        <div
          className={`vd-detail-sheet vd-detail-${activeSheet ?? "closed"} ${activeSheet ? "vd-active" : ""}`}>
                    ...
                    <div className="vd-benefit-item" key={ii}>
                      {item.icon.startsWith("/assets/") ? (
                        <img
                          src={item.icon}
                          alt=""
                          className="vd-benefit-img vd-benefit-img-choose"
                          draggable={false}
                        />
                      ) : (
                        <span>{item.icon}</span>
          ...
          <div className="vd-sheet-footer">
            {buyError && <p className="vd-buy-error">{buyError}</p>}
            <div className="vd-sheet-actions">
              <button
                type="button"
                className={`vd-buy-btn ${currentPlan?.ctaClass ?? ""}`}
                onClick={openPlanInBot}
                disabled={buying || !currentPlan}>
                {buying ? (
                  <span className="vd-buy-loading">CREATING ORDER...</span>
                ) : (
                  <>
                    <span className="vd-buy-label">GET CODE</span>
                    <span className="vd-buy-price">{currentPlan?.stars ?? 0} STARS ⭐</span>
                  </>
                )}
              </button>
              <button
