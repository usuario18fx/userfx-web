...
function CrownSVG() {
  return (
    ...
    >
      <defs>
        earGradient id="crownGrad" x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stopColor="#c8e6ff" />
          <stop offset="40%" stopColor="#6aaee8" />
          <stop offset="100%" stopColor="#1a3f6e" />
        </linearGradient>

        earGradient id="crownShine" x1="0%" y1="0%" x2="100%" y2="0%">
      ...
      <rect x="8" y="38" width="64" height="14" rx="3" fill="url(#crownGrad)" filter="url(#crownGlow)" />
      <rect x="8" y="38" width="64" height="14" rx="3" fill="url(#crownShine)" />

      {[18, 32, 48, 62].map((x) => (
        ircle key={x} cx={x} cy="45" r="3" fill="#4a90d9" stroke="#a8d4ff" strokeWidth="0.8" />
      ))}

      <polygon points="8,38 20,10 32,28" fill="url(#crownGrad)" filter="url(#crownGlow)" />
      <polygon points="24,38 40,4 56,38" fill="url(#crownGrad)" filter="url(#crownGlow)" />
      <polygon points="48,38 60,10 72,38" fill="url(#crownGrad)" filter="url(#crownGlow)" />

      ircle cx="20" cy="11" r="4" fill="#a8d4ff" stroke="#ffffff" strokeWidth="0.8" opacity="0.9" />
      ircle cx="40" cy="5" r="5" fill="#7ec0ff" stroke="#ffffff" strokeWidth="1" opacity="0.95" />
      ircle cx="60" cy="11" r="4" fill="#a8d4ff" stroke="#ffffff" strokeWidth="0.8" opacity="0.9" />
...
function RoseSVG() {
  return (
    <svg viewBox="0 0 160 100" width="180" height="110" className={styles.roseSvg}>
      ...
      <path d="M 75 95 Q 72 70 68 55" stroke="#2d6e30" strokeWidth="3" fill="none" strokeLinecap="round" />

      <ellipse cx="58" cy="72" rx="18" ry="9" fill="#2d6e30" transform="rotate(-35 58 72)" opacity="0.9" />
      <ellipse cx="80" cy="78" rx="14" ry="7" fill="#3a8c3e" transform="rotate(20 80 78)" opacity="0.85" />
      ...
      <ellipse cx="62" cy="42" rx="18" ry="14" fill="#8b001e" transform="rotate(-20 62 42)" opacity="0.8" filter="url(#roseGlow)" />
      <ellipse cx="88" cy="40" rx="18" ry="14" fill="#8b001e" transform="rotate(20 88 40)" opacity="0.8" />
      <ellipse cx="75" cy="35" rx="14" ry="18" fill="#7a001e" opacity="0.7" />

      <ellipse cx="58" cy="48" rx="20" ry="15" fill="url(#petalOuter)" transform="rotate(-15 58 48)" />
      <ellipse cx="92" cy="46" rx="20" ry="15" fill="url(#petalOuter)" transform="rotate(15 92 46)" />
      <ellipse cx="75" cy="42" rx="16" ry="20" fill="#c8002f" opacity="0.9" />

      <ellipse cx="63" cy="52" rx="17" ry="13" fill="url(#petalOuter)" transform="rotate(-8 63 52)" opacity="0.95" />
      <ellipse cx="87" cy="52" rx="17" ry="13" fill="url(#petalOuter)" transform="rotate(8 87 52)" opacity="0.95" />
      ...
      <ellipse cx="68" cy="45" rx="5" ry="3" fill="#ff8fa8" opacity="0.4" transform="rotate(-20 68 45)" />
    </svg>
  );
}
  ...
  return (
    <div className={styles.page}>
      ...
      <main className={styles.main}>
        <div
          className={`${styles.card} ${status === "error" ? styles.cardError : ""} ${status === "unlocked" ? styles.cardUnlocked : ""}`}
        >
          {status !== "unlocked" ? (
            <>
              ...
              {status === "error" && <p className={styles.errorMsg}>Incorrect code. Try again.</p>}

              <button type="button" className={styles.unlockBtn} onClick={() => submitCode(chars.join(""))}>
                UNLOCK ALBUM
              </button>
