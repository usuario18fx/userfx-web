  ...
  if (status === "unlocked") {
    return (
      <div className={styles.unlockedRoot}>
        ...
        {videoSrc ? (
          <video src={videoSrc} controls autoPlay playsInline preload="metadata" className={styles.unlockedVideo}/>
        ) : (
          <div className={styles.unlockedEmpty}>
            <div className={styles.unlockedEmptyIcon}>
              🜲
            </div>
            <span className={styles.unlockedEmptyTitle}>
              𝐀𝐋𝐁𝐔𝐌 𝐔𝐍𝐋𝐎𝐂𝐊𝐄𝐃
            </span>
...
/* ── LOCKED ── */
  return (
    <section className={styles.root}style={{ "--wp": `url(${wallpaper})` }}>
{/* — Background — */}
        ...
        <div className={styles.tabRow}>
          {["FX-USER01-", "AX01-"].map((tab) => (
            <button key={tab} type="button" className={`${styles.tab} ${activeTab === tab ? styles.tabActive : ""}`} onClick={() => setActiveTab(tab)}> 
              {activeTab === tab && <span className={styles.tabDot} />}  {tab}
            </button> ))}
        ...
        <div className={styles.codeRow} onPaste={handlePaste}>
          {chars.map((char, i) => (
            <div key={i} className={styles.codeCell}>
              <input ref={(el) => { inputsRef.current[i] = el; }} type="text" inputMode="text" autoComplete={i === 0 ? "one-time-code" : "off"} maxLength={1} value={char} onChange={(e) => handleChange(i, e.target.value)} onKeyDown={(e) => handleKeyDown(e, i)} className={`${styles.codeBox} ${status === "error" ? styles.codeBoxError : ""}`} aria-label={`Character ${i + 1} of ${CODE_LENGTH}`}/>
...
{/* CTA button */}
        ...
        <p className={styles.hint}>
          ᴇɴᴛᴇʀ ᴛʜᴇ ʟᴀꜱᴛ 4 ᴄʜᴀʀᴀᴄᴛᴇʀꜱ ᴏꜰ ʏᴏᴜʀ ᴄᴏᴅᴇ
        </p>
      </div>
{/* — Footer — */}
