...
if 'const SUPABASE_URL = process.env.SUPABASE_URL || "";' not in s:
    marker = '''const USERFX_SITE_URL = process.env.USERFX_SITE_URL ||\n    "https://userfx-web.vercel.app";'''
    replacement = marker + '''\nconst SUPABASE_URL = process.env.SUPABASE_URL || "";\nconst SUPABASE_SERVICE_ROLE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || "";'''
    if marker not in s:
        raise SystemExit("USERFX_SITE_URL marker not found")
    s = s.replace(marker, replacement, 1)

helper_marker = "function getCommandArg(ctx, index = 0) {"
if "// TELEGRAMFX ACCESS / SUPABASE" not in s:
    if helper_marker not in s:
        raise SystemExit("getCommandArg marker not found")
    helper = r'''// ======================================================
...
command_marker = "// ======================================================\n// ADMIN CODE LOOKUP"
if 'bot.command("access"' not in s:
    if command_marker not in s:
        raise SystemExit("ADMIN CODE LOOKUP marker not found")
    commands = r'''// ======================================================
