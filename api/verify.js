...
function normalizeCodeStatus(value) {
  ...
  if (status === "active" || status === "ᴀᴄᴛɪᴠᴇ") {
    return "active";
  }
...
function getSessionSeconds(planId) {
  if (planId === "vip") return VIP_SESSION_SECONDS;
  if (planId === "pro") return PRO_SESSION_SECONDS;
  return BASIC_SESSION_SECONDS;
}
...
function getRateLimitKey(ip) {
  return `${CODE_ENGINE_NAMESPACE}:verify-rate:${hashValue(ip).slice(0, 24)}`;
}

async function isRateLimited(redis, ip) {
  const attempts = Number((await redis.get(getRateLimitKey(ip))) || 0);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.error("[api/verify/redis]", error);
  ...
  try {
    ...
    try {
      ...
      console.error("[api/verify] Invalid Redis record", { redisKey });
      return res.status(500).json({
        ok: false,
    ...
    const recordPlanId = String(record.planId || "")
      .trim()
      .toLowerCase();

    if (recordPlanId !== expectedPlanId) {
      console.error("[api/verify] Prefix and plan mismatch", {
        redisKey,
        expectedPlanId,
        recordPlanId,
      });
    ...
    const status = normalizeCodeStatus(record.status);
    ...
    return res.status(200).json({
      ...
      code: fullCode,
      planId: recordPlanId,
      plan: record.plan || String(recordPlanId).toUpperCase(),
