...
//// BUTTON TRACKING //
    ...
    await redis.set(`button_click:${user.id}:${Date.now()}`,
...
async function openVideocallFlow(
  ctx: any
) {
  const userId =
    String(ctx.from?.id || "");
...
async function sendPendingVideocallPanel(
  ctx: any
) {
  const userId =
    String(ctx.from?.id || "");
...
async function sendVipInvoice(
  ...
  await ctx.telegram.callApi(
    "sendInvoice",
    {
      ...
      title: "𝓥𝓘𝓟 ᴀᴄᴄᴇss",
      description:
        "𝓥𝓘𝓟 ᴀᴄᴄᴇꜱꜱ ᴡɪᴛʜ ᴛᴇʟᴇɢʀᴀᴍ ꜱᴛᴀʀꜱ.",
      payload: VIP_PAYLOAD,
      currency: "XTR",
      prices: [
        {
...
async function sendBasicInvoice(
  ctx: any
) {
  const chatId =
    ctx.chat?.id ||
...
async function sendProInvoice(
  ...
  await ctx.telegram.callApi(
    "sendInvoice",
    {
      ...
      title: "ℙℝ𝕆 ᴀᴄᴄᴇss",
      description:
        "ᴍᴇɴꜱᴜᴀʟ ᴀᴄᴄᴇꜱꜱ ᴡɪᴛʜ ᴛᴇʟᴇɢʀᴀᴍ ꜱᴛᴀʀꜱ.",
      payload: PRO_PAYLOAD,
      currency: "XTR",
      prices: [
        {
...
//// SUCCESSFUL PAYMENT //
    ...
    const payload = payment.invoice_payload;  logger.info(
      "SUCCESSFUL PAYMENT",
    {userId,
     payload,
     chargeId,});
  if (isVaultPayload(payload)) {const relayed =
...
ʏᴏᴜʀ ${plan.name} ᴀᴄᴄᴇꜱꜱ ɪꜱ ɴᴏᴡ ʀᴇᴀᴅʏ.`,
        getMainKeyboard()
      );
...
Chat ID: ${escapeHtml(userId)}
    ...
    await bot.telegram.copyMessage(
      ADMIN_CHAT_ID, ctx.chat.id, ctx.message.message_id
    );
...
//// USER TEXT ROUTER // 
  ...
  async (ctx) => {
    ...
    const userId = String(ctx.from?.id || ""); if (!userId) return;
    try {
      if ( text.startsWith("/")
      ) {
        return;
...
//// VIDEOCALL //
      ...
      await sendMediaSafe(ctx,"photo",
          ASSET_VIDEOCALL_IMAGE);
      return await openVideocallFlow(ctx);
      }
//// PENDING REQUEST //
      if (
        text === BTN_PENDING_REQUEST
      ) {
...
//// SMOKELANDIA //
      ...
      await sendMediaSafe(ctx,"video",
         ASSET_SMOKELANDIA_VIDEO);
      return await ctx.reply(
        "𝕊ᴍᴏᴋᴇʟᴀɴᴅɪᴀ",
      {reply_markup: {inline_keyboard: 
      [[{text:"ɢᴇᴛ-ɪɴ",
...
//// USERFX SITE // ================================================
      ...
      await sendMediaSafe( ctx,"video",
        ASSET_USERFX_VIDEO); return await ctx.reply(
          "𝐔𝐬ᴇʀ 🜲∓ҳ",
          { reply_markup: { inline_keyboard: [
          [{text:"↗ ᴇɴᴛᴇʀ",
            url: USERFX_SITE_URL,},],],},});
...
//// REPORT //
    ...
    return JSON.parse( value as string);
     ...
     return null;}})
      .filter(Boolean)
      .sort((a, b) => new Date(b.clickedAt).getTime() -
...
//// ERROR HANDLERS //
     bot.catch((error, ctx) => {logger.error(
      ...
      { updateId: ctx?.update?.update_id ?? null,
        chatId: ctx?.chat?.id ?? null,
        userId: ctx?.from?.id ?? null,...getTelegramError(error),
        stack: error instanceof Error ? error.stack : null,
      });
    });
//// RAW BODY READER //
    ...
    (resolve, reject) => {
      ...
      if (bytes >MAX_BODY_BYTES) {reject(new Error(
        "payload_too_large"));
      req.destroy();return;}
      body += chunk;});
...
//// TELEGRAM BODY PARSER //
    function parseTelegramBody(rawBody: any) {
