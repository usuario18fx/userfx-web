...
async function redisGetJson(key) {
  ...
  let cursor = "0";
  try {
  do { const result = await redis.scan(
        cursor,
        "MATCH",
        pattern,
        "COUNT",
...
//// BUTTON TRACKING //
 async function trackButtonClick(ctx, buttonName) {
  ...
  await redis.set(`button_click:${user.id}:${Date.now()}`,JSON.stringify(data),"EX",60 * 60 * 24 * 30
  );
  } catch (error) {
...
async function sendVipPanel(ctx) {
  ...
  } catch (error) {logger.error("VIP PANEL ERROR", {...getTelegramError(error),
  });
  }}
// // USER //
...
ꜱᴇɴᴅ ʏᴏᴜʀ ᴘʜᴏᴛᴏ ʜᴇʀᴇ ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ.`,
    ...
    logger.info("ADMIN REQUEST SEND START", {
  ...
  ᴜꜱᴇʀɴᴀᴍᴇ: ${escapeHtml(user.username)}
  ɪᴅ: ${escapeHtml(user.id)}
  ᴄʜᴀᴛ ɪᴅ: ${escapeHtml(userId)}
  ⏳ᴡᴀɪᴛɪɴɢ ꜰᴏʀ ᴘʜᴏᴛᴏ...`,
    { parse_mode: "HTML",}
    );
  logger.info("ADMIN REQUEST SEND SUCCESS", {
    adminChatId: ADMIN_CHAT_ID,
    userId,
    });
  } catch (error) {
...
async function sendVipInvoice(ctx) {
  ...
  logger.error("USER INVOICE ERROR", {...getTelegramError(error),
  });
  throw error;
  }}
async function sendStars130Invoice(userId) {
  try { await bot.telegram.callApi("sendInvoice",
  {
   ...
   currency: "xᴛʀ",
   prices: [
  {label: "ᴠɪᴅᴇᴏᴄᴀʟʟ ᴀᴄᴄᴇꜱꜱ",
   amount: STARS_130_PRICE,
  },],});
  } catch (error) {
  logger.error(
  "𝟭𝟯𝟬 ꜱᴛᴀʀꜱ ɪɴᴠᴏɪᴄᴇ ᴇʀʀᴏʀ",
  { userId,...getTelegramError(error),
  });
  throw error;
  }}
...
async function handleSuccessfulPayment(ctx) {
  ...
  await getVideoRequest(userId);
    ...
    await ctx.reply(`⚠️ ᴘᴀʏᴍᴇɴᴛ ʀᴇᴄᴇɪᴠᴇᴅ, ʙᴜᴛ ɴᴏ ᴀᴄᴛɪᴠᴇ ᴠɪᴅᴇᴏᴄᴀʟʟ ʀᴇQᴜᴇꜱᴛ ᴡᴀꜱ ꜰᴏᴜɴᴅ.
...
//// USER START //
  ...
  } catch (error) { logger.error("START ERROR", {
...
//// PRE CHECKOUT //
 ...
 async (ctx) => {
    try {
      ...
      logger.error("PRE CHECKOUT ERROR",
  {
    ...getTelegramError(error),
  });
  }});
...
📸 ᴜꜱᴇʀ ᴘʜᴏᴛᴏ ɪꜱ ᴀᴛᴛᴀᴄʜᴇᴅ ʙᴇʟᴏᴡ.`,
  ...
  await ctx.reply(`📸 ᴘʜᴏᴛᴏ ʀᴇᴄᴇɪᴠᴇᴅ.
ᴡᴀɪᴛ ᴡʜɪʟᴇ ᴡᴇ ʀᴇᴠɪᴇᴡ ɪᴛ.`
  );
  } catch (error) {logger.error("MEDIA HANDLER ERROR",
  {userId,
  ...getTelegramError(error),
  stack: error?.stack || null,
  adminChatId:
...
// ======================================================
 ...
 (click, index) => {report +=
  ...
  const chunks = [];
  while (report.length > 0) {chunks.push(
    report.slice(0, 3900)
  );
    report =
    report.slice(3900);}
