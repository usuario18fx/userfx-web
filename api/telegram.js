...
const VAULT_WEBHOOK_SECRET = process.env.VAULT_WEBHOOK_SECRET || process.env.TELEGRAM_WEBHOOK_SECRET ||"";
...
async function validateBotTokens() {
  try { const userMe = await bot.telegram.getMe();
...
if (REDIS_URL) {
  try {
    ...
    } catch (error) { logger.error("REDIS INIT FAILED", {
      message: error?.message,
      stack: error?.stack,
    });
    redis = null;
...
async function redisGetJson(key) {
  ...
  } catch (error) { logger.error("REDIS GET ERROR", { key, message: error?.message,
  });
  return null;
  }
  }
...
async function scanKeys(pattern) {
  ...
  try { do {
    ...
    logger.error(
      "REDIS SCAN ERROR", {pattern, message: error?.message,
    });
    return [];
  }}
// // DATA // 
...
//// BUTTON TRACKING // 
   async function trackButtonClick(ctx, buttonName) {
    ...
    JSON.stringify(data),"EX", 60 * 60 * 24 * 30
    );
    } catch (error) {
...
//// //
    ...
    [BTN_GET_FULL_ACCESS],
    [BTN_VIDEOCALL, BTN_PENDING_REQUEST],
    [BTN_CHANNELS],
    ]).resize();
////  //
    const videocallActiveKeyboard = Markup.keyboard([
    [BTN_VIDEOCALL, BTN_PENDING_REQUEST],
    [BTN_CANCEL],
    [BTN_BACK_MENU],
    ]).resize();
...
async function sendMainPanel(ctx) {
  try {
    ...
    ᴜꜱᴇ ᴛʜᴇ ʙᴜᴛᴛᴏɴꜱ ʙᴇʟᴏᴡ ᴛᴏ ɴᴀᴠɪɢᴀᴛᴇ ᴏᴜʀ ᴘʀɪᴠᴀᴛᴇ ꜱᴇᴄᴛɪᴏɴꜱ.`,getMainKeyboard()
      ...
      logger.error("MAIN PANEL ERROR", { ...getTelegramError(error),
...
async function sendBasicPanel(ctx) {
  try {
   ...
   ⇀ ʙᴇɴᴇғɪᴛs
   ⇀ ʙᴇɴᴇғɪᴛs
   ⇀ ᴡᴇᴇᴋ¹ / ᴀʟʙᴜᴍ¹`, getStarsUserKeyboard()
    );
    } catch (error) {
...
//// APPROVED VIDEOCALL //
  ...
  try {
    ...
    "SEND APPROVED VIDEOCALL ERROR",
      {userId: targetUserId,...getTelegramError(error),
      }
      );
      throw error;
      }
  ...
  try { await ctx.telegram.callApi("sendInvoice",
     ...
     {label: "𝓥𝓘𝓟 ᴀᴄᴄᴇss",
      amount: VIP_STARS_PRICE,},],}
        ...
        "𝓥𝓘𝓟  INVOICE ERROR", {...getTelegramError(error),
      });
      throw error; 
      }}
  ...
  try {
    await ctx.telegram.callApi(
      ...
      {chat_id: chatId,
       title: "𝔹𝔸𝕊𝕀ℂ ᴀᴄᴄᴇss",
       description:
        "𝔹𝔸𝕊𝕀ℂ ᴀᴄᴄᴇꜱꜱ ᴡɪᴛʜ ᴛᴇʟᴇɢʀᴀᴍ ꜱᴛᴀʀꜱ.",
      ...
      {label: "𝔹𝔸𝕊𝕀ℂ ᴀᴄᴄᴇss",
       amount: USER_STARS_PRICE,},],}
        ...
        "𝔹𝔸𝕊𝕀ℂ INVOICE ERROR", {...getTelegramError(error),
      });
     throw error;
     }}
async function sendPROInvoice(ctx) { const chatId =
    ctx.chat?.id ||
    ctx.callbackQuery?.message?.chat?.id;
  if (!chatId) {
    return;
  ...
  try {
    await ctx.telegram.callApi(
      ...
      {chat_id: chatId,
       title: "ℙℝ𝕆 ᴀᴄᴄᴇss",
       description: 
       "ᴍᴇɴsᴜᴀʟ ᴀᴄᴄᴇꜱꜱ ᴡɪᴛʜ ᴛᴇʟᴇɢʀᴀᴍ ꜱᴛᴀʀꜱ.",
       payload: PRO_PAYLOAD,
       currency: "XTR",
       prices: [
      {label: "ℙℝ𝕆 ᴀᴄᴄᴇss",
    ...
    "PRO INVOICE ERROR", { ...getTelegramError(error),
       });
    throw error;
      }}
    ...
    await bot.telegram.callApi(
      ...
      { chat_id: String(userId),
        ...
        "ᴀᴄᴄᴇꜱꜱ ᴛᴏ ᴛʜᴇ ᴠɪᴅᴇᴏᴄᴀʟʟ ꜱᴇʀᴠɪᴄᴇ.",
          payload: STARS_130_PAYLOAD, currency: "XTR", prices: [
          {label: "ᴠɪᴅᴇᴏᴄᴀʟʟ ᴀᴄᴄᴇss",
          amount: STARS_130_PRICE,},], }  );
  } catch (error) {
    logger.error(
      "✪𝟭𝟯𝟬 STARS INVOICE ERROR",
      { userId,  ...getTelegramError(error),});
    throw error;
      }}
//// SUCCESSFUL PAYMENT //
...
// // VIDEOCALL 130 STARS // 
  ...
  //// PRO //
    if (payload === PRO_PAYLOAD) {
      ...
      await markPaymentProcessed(chargeId);
      await ctx.reply(
          `✅ ᴍᴏɴᴛʜʟʏ ᴀᴄᴛɪᴠᴀᴛᴇᴅ\n    ʏᴏᴜʀ ᴍᴏɴᴛʜʟʏ ᴀᴄᴄᴇꜱꜱ ɪꜱ ɴᴏᴡ ᴜɴʟᴏᴄᴋᴇᴅ.`, getMainKeyboard()
          );
      return;
           }
  //// USER //
      ...
      await ctx.reply(
        `✅ "𝔹𝔸𝕊𝕀ℂ" ᴀᴄᴛɪᴠᴀᴛᴇᴅ
      ʏᴏᴜʀ "𝔹𝔸𝕊𝕀ℂ" ᴀᴄᴄᴇꜱꜱ ɪꜱ ɴᴏᴡ ᴜɴʟᴏᴄᴋᴇᴅ.`,getMainKeyboard()
      );}}
//// PENDING VIDEOCALL CALLBACK //
      bot.action(/^pending_videocall_(\d+)$/,
      async (ctx) => {
...
// Seguridad: el usuario solo puede consultar su propia solicitud
      ...
      let message = ""; switch (request.status) {
      case REQUEST_STATUS.WAITING_PHOTO:
          message =
      ...
      case REQUEST_STATUS.AWAITING_PAYMENT:
          message =
            ...
            "ᴘʟᴇᴀꜱᴇ ᴄᴏᴍᴘʟᴇᴛᴇ ᴛʜᴇ ✪𝟭𝟯𝟬 ꜱᴛᴀʀꜱ ᴘᴀʏᴍᴇɴᴛ.";
      break;
      case REQUEST_STATUS.PAID:
          message =
      ...
      await ctx.reply(message); logger.info(
...
bot.start(async (ctx) => {
  try {
    ...
    "START ERROR", {...getTelegramError(error), stack: error?.stack,
       });
       }});
//// SUPPORT //
...
/////////////////
      bot.action("pay_user_stars",
    async (ctx) => {
    try {
      await ctx.answerCbQuery();
      await sendUserInvoice(ctx);
    } catch (error) {logger.error(
      "PAY USER ERROR", {...getTelegramError(error),
      });
     }});
//// BACK MAIN // 
...
//// Ignore delete errors
      ...
      "BACK MAIN ERROR", {...getTelegramError(error),
      });
     }});
//// PRE CHECKOUT //
...
const userId = String( ctx.from?.id || ""
  ...
  try {
    ...
    await bot.telegram.copyMessage(ADMIN_CHAT_ID,
      ctx.chat.id,
    ...
    await ctx.reply(
     ...
     ᴡᴀɪᴛ ᴡʜɪʟᴇ ʀᴇᴠɪᴇᴡ ɪᴛ.`
      ...
      "MEDIA HANDLER ERROR",
    {userId,...getTelegramError(error), stack: error?.stack || null, adminChatId: ADMIN_CHAT_ID || null,}
    );
    }}
// Handle user photos //
...
//// VIDEOCALL // 
      ...
      const existing = await redis.get(`video_request:${userId}`);
      if (existing) {
      return ctx.reply(
      "⏳ ʏᴏᴜ ᴀʟʀᴇᴀᴅʏ ʜᴀᴠᴇ ᴀ ᴘᴇɴᴅɪɴɢ ʀᴇǫᴜᴇꜱᴛ",
      videocallActiveKeyboard
      );
      }
...
//// PENDING REQUEST ///
     bot.hears(BTN_PENDING_REQUEST, async (ctx) => {
     const userId = ctx.from.id;
     const data = await redis.get(`video_request:${userId}`);
     if (!data) {
     return ctx.reply("❌ ɴᴏ ᴘᴇɴᴅɪɴɢ ʀᴇǫᴜᴇꜱᴛ");
     }
    const request = JSON.parse(data);
    return ctx.reply(
    `⏳ ꜱᴛᴀᴛᴜꜱ: ${request.status}`,
    videocallActiveKeyboard
    );
   });
//// HANDLE CANCEL ///
       ...
       const userId = ctx.from.id;
       await redis.del(`video_request:${userId}`);
       return ctx.reply(
         "✖ ʀᴇǫᴜᴇꜱᴛ ᴄᴀɴᴄᴇʟᴇᴅ",mainKeyboard
      );
      });
//// CLEAR VIDEO // 
...
// // RESET VIDEOCALL REQUEST //
      ...
      logger.info(
      "VIDEOCALL REQUEST RESET",
      {userId, previousStatus: request.status || null,}
      );
    } catch (error) {logger.error(
        "RESET VIDEOCALL ERROR",
      {userId,...getTelegramError(error),stack: error?.stack,}
      );
      await ctx.reply(
...
//// ADMIN MYID //
adminBot.command("myid", async (ctx) => {try { await ctx.reply(
      `chat_id: ${ctx.chat?.id}\nuser_id: ${ctx.from?.id}`
      );
      } catch (error) {logger.error(
        "ADMIN MYID ERROR",
...
//// ADMIN APPROVE STARS //
adminBot.action(/^approve_stars_(\d+)$/,
   async (ctx) => {
    ...
    await setVideoRequest( requesterId,
      { ...current,  status: REQUEST_STATUS.AWAITING_ADMIN,
      });
      }
      try { await bot.telegram.sendMessage( requesterId,
...
adminBot.action(
  ...
  async (ctx) => {
    ...
    try {
      ...
      await bot.telegram.sendMessage( requesterId,
        `⏳ ʜᴇʏ ꜱᴇxʏ, ɪ’ᴍ ᴏɴ ᴀ ᴠɪᴅᴇᴏ ᴄᴀʟʟ ᴡɪᴛʜ ᴀɴᴏᴛʜᴇʀ 
        ɢᴜʏ ʀɪɢʜᴛ ɴᴏᴡ. ɪꜱ ɪᴛ ᴄᴏᴏʟ ɪꜰ ɪ ᴛᴇxᴛ ʏᴏᴜ ᴡʜᴇɴ ɪ’ᴍ ғʀᴇᴇ?.   ³・³`,
...
//// ERROR HANDLER //
     bot.catch((error, ctx) => { logger.error(
      "BOT ERROR",
     {updateId: ctx?.update?.update_id ?? null,
      ...getTelegramError(error),
      stack: error?.stack || null,
      });});
      adminBot.catch((error, ctx) => {logger.error(
        "ADMIN BOT ERROR",
      {updateId: ctx?.update?.update_id ?? null,
       chatId: ctx?.chat?.id ?? null,
       userId: ctx?.from?.id ?? null,...getTelegramError(error),stack: error?.stack || null,
      });
      });
///// RAW BODY READER //
function readRawBody(req) {
  return new Promise(
    (resolve, reject) => {let body = "";
