...
function getPlanDisplay(userId) {
  ...
  if (!membership) {
    return { label: "[X-user]", price: "$3", access: "Premium", status: "Inactive", emoji: "👑", planKey: "user" };
  }
  if (membership.planKey === "vip") {
    return { label: "[V-vip]", price: "$12", access: "Unlimited", status: "Active", emoji: "🔥", planKey: "vip" };
  }
  return { label: "[X-user]", price: "$3", access: "Premium", status: "Active", emoji: "👑", planKey: "user" };
}
...
async function notifyAdminNewRequest(ctx) {
  ...
  await bot.telegram.sendMessage(
    ...
    { parse_mode: "HTML", ...getAdminVideoRequestInlineKeyboard(requester.id) }
  );
}
...
async function notifyAdminMediaReceived(ctx, label = "Media") {
  ...
  await bot.telegram.sendMessage(
    ...
    { parse_mode: "HTML", ...getAdminApprovalKeyboard(requester.id) }
  );
}
...
async function sendVipMode(ctx) {
  await ctx.reply(buildVipCard(ctx.from?.id), { ...getVipPaymentInlineKeyboard() });
  await ctx.reply("‎", getBackKeyboard());
}

async function sendChannelsPanel(ctx) {
  await ctx.reply(`📺\nᴄʜᴀɴɴᴇʟꜱ`, { ...getChannelsInlineKeyboard() });
  await ctx.reply("‎", getBackKeyboard());
}

async function sendWebsitePanel(ctx) {
  await ctx.reply(`🌐\nᴡᴇʙꜱɪᴛᴇ`, { ...getWebsiteInlineKeyboard() });
  await ctx.reply("‎", getBackKeyboard());
}
...
async function sendCloudsMessage(ctx) {
  await ctx.replyWithVideo(Input.fromLocalFile(asset("welcome-smkl.mp4")), {
    caption: `ᴄʟᴏᴜᴅꜱ\n☁️\nᴀᴍʙɪᴇɴᴛ ʀᴏᴏᴍ`,
    ...getWebsiteInlineKeyboard(),
  });
  await ctx.reply("‎", getAccessKeyboard());
...
async function startVideoCallFlow(ctx) {
  ...
  if (!userId) { await ctx.reply("Unable to identify your account."); return; }
...
async function sendSelectedPlatformLink(ctx, platform) {
  ...
  if (platform === "zoom") {
    await ctx.reply(
      ...
      { reply_markup: { inline_keyboard: [[{ text: "🔷 ENTER ZOOM", url: ZOOM_URL }]] } }
    );
  } else {
    await ctx.reply(
      `📞 ᴛᴇʟᴇɢʀᴀᴍ ꜱᴇʟᴇᴄᴛᴇᴅ\nᴛᴀᴘ ʙᴇʟᴏᴡ ᴛᴏ ᴏᴘᴇɴ ʏᴏᴜʀ ᴠɪᴅᴇᴏᴄᴀʟʟ ʟɪɴᴋ.`,
      { reply_markup: { inline_keyboard: [[{ text: "📞 OPEN TELEGRAM", url: TELEGRAM_CALL_URL }]] } }
    );
  }
...
bot.start(async (ctx) => {
  ...
  if (payload === "videocall") { await startVideoCallFlow(ctx); return; }
  if (payload === "userchannel") {
    await ctx.reply(`🜲 ᴜꜱᴇʀ ᴇɴᴛʀʏ`, {
      reply_markup: { inline_keyboard: [[{ text: "📺 OPEN USER CHANNEL", url: USER_GROUP_LINK }]] },
    });
    return;
  }
  if (payload === "smokelandiachannel") {
    await ctx.replyWithVideo(Input.fromLocalFile(asset("welcome-smkl.mp4")), {
      caption: `☁️\nꜱᴍᴏᴋᴇʟᴀɴᴅɪᴀ ᴇɴᴛʀʏ`,
      reply_markup: { inline_keyboard: [[{ text: "📺 OPEN SMOKELANDIA", url: SMOKELANDIA_GROUP_LINK }]] },
    });
    return;
  }
...
bot.command("help", async (ctx) => {
  await ctx.reply("ʜᴇʟᴘ 🆘", getMainKeyboard());
});
...
bot.hears(BTN_VIDEOCALL, async (ctx) => { await startVideoCallFlow(ctx); });
bot.hears(BTN_CHANNELS,  async (ctx) => { await sendChannelsPanel(ctx); });
bot.hears(BTN_WEBSITE,   async (ctx) => { await sendWebsitePanel(ctx); });
bot.hears(BTN_REFRESH,   async (ctx) => { await sendMainMenu(ctx); });
bot.hears(BTN_FEED,      async (ctx) => { await sendFeedMessage(ctx); });
bot.hears(BTN_CLOUDS,    async (ctx) => { await sendCloudsMessage(ctx); });
bot.hears(BTN_PHOTOS,    async (ctx) => { await sendPhotosMessage(ctx); });
bot.hears(BTN_GIFTS,     async (ctx) => { await sendGiftsMessage(ctx); });
bot.hears(BTN_BACK,      async (ctx) => { await sendMainMenu(ctx); });
bot.hears(BTN_EXIT,      async (ctx) => { await sendMainMenu(ctx); });
...
bot.action("buy_user_stars", async (ctx) => {
  ...
  await ctx.replyWithInvoice({
    title: "X-user", description: "Premium access", payload: "membership_user",
    currency: "XTR", prices: [{ label: "X-user", amount: 300 }],
    provider_token: "", start_parameter: "buy-user-stars",
  });
});
...
bot.action("buy_vip_stars", async (ctx) => {
  await ctx.answerCbQuery();
  await ctx.replyWithInvoice({
    title: "V-vip", description: "Unlimited access", payload: "membership_vip",
    currency: "XTR", prices: [{ label: "V-vip", amount: 1200 }],
    provider_token: "", start_parameter: "buy-vip-stars",
  });
});
...
bot.action("choose_platform_zoom", async (ctx) => {
  ...
  if (!pending?.waitingForPlatform) { await ctx.reply("This videocall request is no longer active."); return; }
...
bot.action(/^approve_video_(.+)$/, async (ctx) => {
  ...
  if (!pending) { await ctx.reply("Request no longer exists."); return; }
...
ᴄʜᴏᴏꜱᴇ ʏᴏᴜʀ ᴠɪᴅᴇᴏ ᴘʟᴀᴛꜰᴏʀᴍ ʙᴇʟᴏᴡ 👇
- ╩————————————╩•`,
      reply_markup: {
        inline_keyboard: [
          [
...
bot.action(/^reject_video_(.+)$/, async (ctx) => {
  ...
  await bot.telegram.sendMessage(
    requesterId,
    `•╦————————————╦•
❌ ʀᴇQᴜᴇꜱᴛ ɴᴏᴛ ᴀᴘᴘʀᴏᴠᴇᴅ

ʀᴇᴛᴜʀɴ ᴛᴏ ᴍᴇɴᴜ.
- ╩————————————╩•`
  );
  await bot.telegram.sendMessage(requesterId, buildWelcomeCaption(), getMainKeyboard());
});
...
bot.on("successful_payment", async (ctx) => {
  ...
  const userId = String(ctx.from?.id || "");
  setMembership(userId, payload === "membership_vip" ? "vip" : "user");
  await ctx.reply(`•╦————————————╦•
✅ ᴘᴀʏᴍᴇɴᴛ ʀᴇᴄᴇɪᴠᴇᴅ
...
bot.on("text", async (ctx) => {
  ...
  const knownInputs = [
    "/start", "/help", "/videocall",
    BTN_USER, BTN_VIP, BTN_VIDEOCALL, BTN_CHANNELS,
    BTN_WEBSITE, BTN_REFRESH, BTN_FEED, BTN_CLOUDS,
    BTN_PHOTOS, BTN_GIFTS, BTN_BACK, BTN_EXIT, BTN_CANCEL_VIDEO,
  ...
  if (pending?.waitingForMedia) {
    ...
    await ctx.reply("ꜱᴇɴᴅ ᴏɴᴇ ᴘʜᴏᴛᴏ ᴏʀ ᴠɪᴅᴇᴏ ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ.");
    return;
  }
});
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    return res.status(200).json({ ok: true, message: "Telegram endpoint alive", ip });
  }

  if (req.method !== "POST") {
    return res.status(405).json({ ok: false, error: "method_not_allowed", ip });
  }
