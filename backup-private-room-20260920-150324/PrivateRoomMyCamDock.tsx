...
export default function PrivateRoomMyCamDock():ReactElement | null {
  ...
  const handleSnapSide = useCallback(() => {
    ...
    const next = clampPosition(nextRight ? window.innerWidth - rect.width - EDGE_GAP : EDGE_GAP,rect.top,
    );
    setRight(nextRight);
    setPosition(next);
    savePosition(next);
  },[clampPosition,right,savePosition]);
  const handleManage = useCallback(() => {setDocked(false);
    document.body.classList.remove("pvr-camera-docked");
    openCameraStudio();
  },[]);
  const handleVisibility = useCallback(() => {
    ...
    button?.click(); setVisibility(next); },[visibility]);
  ...
  if (minimized) {
    return (
      <button ref={(node) => {dockRef.current = node;}} type="button" className={`pvr-mycam-mini ${right ? "is-right" : "is-left"}${dragging ? " is-dragging" : ""}`} style={dockStyle} onPointerDown={handleDragStart} onPointerMove={handleDragMove} onPointerUp={handleDragEnd} onPointerCancel={handleDragEnd} onClick={() => { if (dragMovedRef.current) { dragMovedRef.current = false; return;} setMinimized(false);}}>
      ...
      <small>
        {visibility.toUpperCase()} · DRAG ANYWHERE</small>
      </span>
      </button>
      );
      }
  return (
      <aside ref={(node) => {dockRef.current = node;}} className={`pvr-mycam-dock ${right ? "is-right" : "is-left"}${dragging ? " is-dragging" : ""}`} style={dockStyle} aria-label="My camera default view">
{/* ─────   DRAG HANDLE ─────── */}
      <header className="pvr-mycam-head" onPointerDown={handleDragStart} onPointerMove={handleDragMove} onPointerUp={handleDragEnd} onPointerCancel={handleDragEnd}>
      <div>
      <span className="pvr-mycam-live-dot"></span>
      <strong>
        MY CAM · DRAG ANYWHERE
      </strong>
      </div>
...
{/* ─────   CAMERA PREVIEW ─────── */}
      ...
      <span className="pvr-mycam-placeholder">
        FX
      </span>
        )}
...
{/* ========   PRIVATE / PUBLIC SWITCH =========================== */}
      ...
      <button type="button"className={`pvr-privacy-switch ${visibility === "public" ? "is-public" : "is-private"}`} onClick={handleVisibility} aria-label={`Camera visibility: ${visibility}`}>
      <span className="pvr-privacy-label pvr-privacy-label--private">
        PRIVATE
      </span>
      <span className="pvr-privacy-track">
      <span className="pvr-privacy-lock">
      <img src={visibility === "private"
        ? "/assets/iconos/priv.png"
        : "/assets/iconos/pub.png"} alt=""  draggable={false}/>
      </span>
      </span>
      <span className="pvr-privacy-label pvr-privacy-label--public">
        PUBLIC
      </span>
      </button>
{/* ─────   MANAGE CAMERA ─────── */}
      <button type="button" className="pvr-mycam-iconBtn" onClick={handleManage} aria-label="Manage camera">
      <img src="/assets/iconos/rec.png" alt="" draggable={false}/>
      </button>
{/* ─────   PROFILE ─────── */}
      <button type="button" className="pvr-mycam-iconBtn" onClick={openProfileEditor} aria-label="Profile">
      <img src="/assets/iconos/user.png" alt="" draggable={false}/>
      </button>
{/* ─────   CAMERA OFF ─────── */}
      <button type="button" className="pvr-mycam-iconBtn is-end" onClick={handleStop} aria-label="Turn camera off">
      <span>
        ×
      </span>
      </button>
