...
function CameraStudio({
  ...
  return (
    <>
      <button type="button"className="pvr-camera-backdrop"onClick={onClose} aria-label="Close camera studio">
      </button>
      <section className="pvr-camera-studio" aria-label="USER FX camera studio">
        ...
        <div className={`pvr-camera-preview ${cameraEnabled ? "" : "is-camera-off"}`}>
          ...
          {requesting && (
            <div className="pvr-camera-placeholder">
              <strong>
              CONNECTING CAMERA</strong>
              <span>
                ALLOW CAMERA + MICROPHONE ACCESS</span>
            </div>
          )}
          {!requesting && error && (
          ...
          {!requesting && !error && !stream && (
            <div className="pvr-camera-placeholder">
              ...
              <span>
                CONNECT YOUR CAMERA TO CONTINUE
              </span>
            </div>
          )}
          {!requesting && !error && stream && !cameraEnabled && (
            <div className="pvr-camera-placeholder">
              ...
              <span>
              TURN CAMERA ON TO RESTORE VIDEO
              </span>
            </div>
          )}
        </div>
        <div className="pvr-camera-layout">
          ...
          <div className="pvr-camera-settings">
            <span>
              WHO CAN SEE YOUR CAM
            </span>
            <div className="pvr-camera-visibility">
              <button type="button" className={visibility === "private" ? "is-active" : ""} onClick={() => onVisibilityChange("private")}>
                PRIVATE
              </button>
              <button type="button" className={visibility === "public" ? "is-active" : ""} onClick={() => onVisibilityChange("public")}>
                PUBLIC
              </button>
            </div>
            <div className="pvr-camera-device-controls">
              <button type="button" className={cameraEnabled ? "" : "is-off"} onClick={onToggleCamera} disabled={!stream}>
                CAMERA {cameraEnabled ? "ON" : "OFF"}
              </button>
              <button type="button" className={micEnabled ? "" : "is-off"} onClick={onToggleMic} disabled={!stream}>
          ...
          <div className="pvr-camera-actions">
            ...
            {error ? (
              ...
              <button type="button" className="button-with-icon pvr-camera-start pvr-camera-enter" onClick={onStart} disabled={!stream || !cameraEnabled || requesting} aria-label="Start camera">
              ...
              <strong>
                {visibility.toUpperCase()} MODE
              </strong>
              <span>
                CAMERA + MICROPHONE ARE CONTROLLED FROM THIS SESSION.
                </span>
              </footer>
              </section>
    </>
          ...
          >
            ...
            <form className="pvr-account-form" onSubmit={handleSave}>
              <label>
                ...
                <input type="text" name="displayName" value={profile.displayName} onChange={handleTextChange} placeholder="Your name" maxLength={40}autoComplete="off"/>
              </label>
              <label>
                <span>
                  BIO
                </span>
                <textarea name="bio" value={profile.bio} onChange={handleTextChange}placeholder="Private profile bio..." maxLength={280} rows={4}/>
